...
std::unique_ptr<Vertices> SkillTreeDBManager::getVertices() {
    ...
    const char* sql =
        "SELECT vertices.vertex_id, vertex_type.type_name, vertices.vertex_name FROM vertices "
        "INNER JOIN vertex_type ON vertices.vertex_type = vertex_type.type_id;";
    sqlite3_stmt* stmt;
    sqlite3_prepare_v2(this->db, sql, -1, &stmt, nullptr);
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        std::size_t id = sqlite3_column_int(stmt, 0);
        Vertex vertex{.id = static_cast<size_t>(sqlite3_column_int(stmt, 0)),
                      .type = fromString(static_cast<std::string>(
